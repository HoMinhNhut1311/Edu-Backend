...
public class PageServiceImpl implements PageService {
    ...
    public Page<UserDtoResponse> getPageUserByRoleId(int pageNumber, int pageSize, int roleId) {
        PageRequest request = PageRequest.of(pageNumber, pageSize);
        if (roleId == 0) {
            // Lấy hết
            return this.userRepository.findAll(request).map(userMapper::mapToResponese);
        } else {
            // Lấy Role từ Role Id
            Role role = this.roleRepository.findById(roleId).orElseThrow(() -> new AppException(errorType.notFound));
            return this.userRepository.getUsersByRoleId(roleId, request).map(userMapper::mapToResponese);
        }
    }
}
